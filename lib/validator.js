function Validator() {
  ...
  function validateFields (fieldMap) {
    ...
    operandList.forEach(function(element, index, array) {
      if((fieldMap[element] || fieldMap[element] === '') && !validateOperand(fieldMap[element])) { invalidFields.push(element) };
    });
    operatorList.forEach(function(element, index, array) {
      if((fieldMap[element] || fieldMap[element] === '') && !validateOperator(fieldMap[element])) { invalidFields.push(element) };
    });
